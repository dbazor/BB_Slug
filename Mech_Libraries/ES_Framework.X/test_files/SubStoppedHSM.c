...
static SubStoppedState_t CurrentState = InitPSubState;
...
uint8_t InitSubStoppedHSM(void)
{
    ...
    returnEvent = RunSubStoppedHSM(INIT_EVENT);
...
/****************************************************************************
 Function
    RunSubStoppedHSM, rename to relevant state machine in your own files
...
ES_Event RunSubStoppedHSM(ES_Event ThisEvent)
{
    ...
    case Halted: // Roach is stopped in darkenss, but got bumped
        switch (ThisEvent.EventType) {
        case ES_ENTRY:
            RoachMotorsStop();
            break;

        case ES_EXIT: // set a 1 1/2 second timer
            ES_Timer_InitTimer(FANCY_ROACH_TIMER, 1500);
            break;

        case BUMPED: // start fleeing
            if (ThisEvent.EventParam | FORWARD_BUMPSWITCHES) {
                nextState = Reversing;
            } else {
    ...
    case Reversing: // Roach is running from light
        ...
        case BUMPED: // only respond to change in bump direction
            if (ThisEvent.EventParam | REARWARD_BUMPSWITCHES) {
                nextState = Forwarding;
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
            }
    ...
    case Forwarding: // Roach is running from light
        switch (ThisEvent.EventType) {
        case ES_ENTRY:
            RoachMotorsForward();
            break;
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunSubStoppedHSM(EXIT_EVENT);
        CurrentState = nextState;
        RunSubStoppedHSM(ENTRY_EVENT);
    }
