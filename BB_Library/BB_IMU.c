...
//    printf(" PAGE ID \n"); 
    ...
    dat = 0x16;
    BB_I2C_Write(BNO55_I2C_ADDR, BNO055_UNIT_SEL, &dat);
    //MPU_I2C_Write(BNO55_I2C_ADDR, BNO055_UNIT_SEL, 1, &dat);
//    printf(" UNIT SEL \n"); 
    
    // Select BNO055 gyro temperature source
    dat = 0x01;
    BB_I2C_Write(BNO55_I2C_ADDR, BNO055_TEMP_SOURCE, &dat);
    //MPU_I2C_Write(BNO55_I2C_ADDR, BNO055_TEMP_SOURCE, 1, &dat);
//    printf(" TEMP SOURCE \n");
//    
    // Select BNO055 system power mode
    dat = 0x00;
    BB_I2C_Write(BNO55_I2C_ADDR, BNO055_PWR_MODE, &dat);
...
//        printf( " in loop %d OPR MODE \n",i);
        BB_I2C_Write(BNO55_I2C_ADDR, OPR_MODE, &dat);
        Delayms(50);
        BB_I2C_Read(BNO55_I2C_ADDR, OPR_MODE, &dat);
        if (dat == NDOF_CON) {
            printf("Configured\n");
